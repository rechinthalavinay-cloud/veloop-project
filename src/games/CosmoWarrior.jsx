...
export function CosmoWarrior({ onOutcome, reviveSignal }) {
  ...
  useEffect(() => {
    ...
    const W = 360, H = 520;
    const s = initState(stateRef.current?.stars);
    
    // Preserve level and score if respawning from pause/continue
    if (stateRef.current) {
       s.score = stateRef.current.score || 0;
       s.level = stateRef.current.level || 1;
       s.lives = stateRef.current.lives || 3;
    }
    stateRef.current = s;

    // DOM Reference HUD Updater (No React state lag!)
    const updateHud = () => {
      const scoreEl = document.getElementById('game-score');
      if (scoreEl) scoreEl.innerText = `⚡ ${s.score}`;
      const livesEl = document.getElementById('game-lives');
      if (livesEl) livesEl.innerText = "♥".repeat(Math.max(0, s.lives)).padEnd(3, "♡");
      const levelEl = document.getElementById('game-level');
      if (levelEl) levelEl.innerText = `Lv ${s.level}`;
    };
    ...
    const onPtr = (e) => {
      // e.preventDefault() prevents scrolling on mobile when swiping
      if (e.cancelable) e.preventDefault();
      const rect = canvas.getBoundingClientRect();
      const pt = e.touches ? e.touches[0] : e;
      s.x = Math.max(10, Math.min(W - 10, ((pt.clientX - rect.left) / rect.width) * W));
      if ((e.type === "pointerdown" || e.type === "touchstart") && s.cooldown <= 0) {
        s.shots.push({ x: s.x - 8, y: H - 60 }); 
        s.shots.push({ x: s.x + 8, y: H - 60 }); 
        s.cooldown = 8;
      }
    ...
    canvas.addEventListener("touchstart", onPtr, { passive: false });
    canvas.addEventListener("touchmove", onPtr, { passive: false });
    ...
    const loop = () => {
      ...
      for (const a of s.aliens) {
        ...
        s.shots = s.shots.filter(sh => {
          ...
          if (Math.hypot(sh.x - a.x, sh.y - a.y) < a.r + 4) {
            hit = true;
        ...
        if (hit) {
          ...
          if (a.hp <= 0) {
            ...
            if (s.score >= target) {
              s.level++;
              if (s.level > 5) {
                setHud({ score: s.score, lives: s.lives, level: s.level }); // Final React update
                phaseRef.current = "complete"; setPhase("complete");
                setTimeout(() => onOutcome?.({ type: "complete", score: s.score }), 0); return;
              }
            }
            updateHud(); // Performant DOM update
            continue; 
          }
        }
        
        // Alien hits player body
        if (a.y > H - 70 && a.y < H - 20 && Math.abs(a.x - s.x) < a.r + 15) {
           s.shake = 15;
           s.lives--;
           updateHud();
           if (s.lives <= 0) {
      ...
      s.shots.forEach(sh => {
        ...
        rGrad.addColorStop(0, "#757575"); rGrad.addColorStop(0.5, "#ffffff"); rGrad.addColorStop(1, "#424242");
        ctx.fillStyle = rGrad;
      ...
      s.alienShots.forEach(sh => {
        ...
        glow.addColorStop(0, "rgba(255, 23, 68, 1)"); glow.addColorStop(1, "rgba(255, 23, 68, 0)");
        ctx.fillStyle = glow;
      ...
      s.aliens.forEach(a => {
        ...
        if (a.type === 'boss') {
          ctx.fillStyle = "#4a148c"; ctx.beginPath(); ctx.moveTo(0,-20); ctx.lineTo(25,10); ctx.lineTo(10,20); ctx.lineTo(-10,20); ctx.lineTo(-25,10); ctx.closePath(); ctx.fill();
          ctx.fillStyle = "#ffb300"; ctx.beginPath(); ctx.moveTo(0,-15); ctx.lineTo(15,5); ctx.lineTo(0,10); ctx.lineTo(-15,5); ctx.closePath(); ctx.fill();
          ctx.fillStyle = "#00e5ff"; ctx.beginPath(); ctx.arc(0, 5, 6, 0, Math.PI*2); ctx.fill();
        } else if (a.type === 'shooter') {
          ctx.fillStyle = "#c62828"; ctx.beginPath(); ctx.moveTo(0,-12); ctx.lineTo(15,0); ctx.lineTo(15,10); ctx.lineTo(0,5); ctx.lineTo(-15,10); ctx.lineTo(-15,0); ctx.closePath(); ctx.fill();
          ctx.fillStyle = "#ff9800"; ctx.beginPath(); ctx.arc(0, 0, 4, 0, Math.PI*2); ctx.fill();
        } else {
          ctx.fillStyle = "#2e7d32"; ctx.beginPath(); ctx.arc(0, 0, 12, Math.PI, 0); ctx.lineTo(10, 10); ctx.lineTo(-10, 10); ctx.closePath(); ctx.fill();
          ctx.fillStyle = "#69f0ae"; ctx.beginPath(); ctx.arc(0, 2, 3, 0, Math.PI*2); ctx.fill();
        }
        ctx.restore();
      });
      ...
      cylGrad.addColorStop(0, "#9e9e9e"); cylGrad.addColorStop(0.3, "#ffffff"); cylGrad.addColorStop(0.8, "#e0e0e0"); cylGrad.addColorStop(1, "#757575");
      const boosterGrad = ctx.createLinearGradient(-4, 0, 4, 0);
      boosterGrad.addColorStop(0, "#9e9e9e"); boosterGrad.addColorStop(0.3, "#ffffff"); boosterGrad.addColorStop(0.8, "#e0e0e0"); boosterGrad.addColorStop(1, "#757575");
      ...
      ctx.fillStyle = s.tick % 4 < 2 ? "#ff9800" : "#ffeb3b";
      ctx.beginPath(); ctx.moveTo(-3, 22); ctx.lineTo(3, 22); ctx.lineTo(0, 35 + Math.random()*10); ctx.fill(); 
      ctx.beginPath(); ctx.moveTo(-13, 18); ctx.lineTo(-9, 18); ctx.lineTo(-11, 28 + Math.random()*8); ctx.fill(); 
      ctx.beginPath(); ctx.moveTo(9, 18); ctx.lineTo(13, 18); ctx.lineTo(11, 28 + Math.random()*8); ctx.fill(); 

      ctx.shadowColor = "transparent";
      ctx.shadowBlur = 0;

      ctx.fillStyle = "#424242";
      ctx.beginPath(); ctx.moveTo(-3, 16); ctx.lineTo(3, 16); ctx.lineTo(4, 22); ctx.lineTo(-4, 22); ctx.fill(); 
      ctx.beginPath(); ctx.moveTo(-12, 12); ctx.lineTo(-10, 12); ctx.lineTo(-9, 18); ctx.lineTo(-13, 18); ctx.fill(); 
      ctx.beginPath(); ctx.moveTo(10, 12); ctx.lineTo(12, 12); ctx.lineTo(13, 18); ctx.lineTo(9, 18); ctx.fill(); 

      ctx.fillStyle = "#757575";
      ctx.fillRect(-10, 2, 6, 2); ctx.fillRect(4, 2, 6, 2); ctx.fillRect(-10, 8, 6, 2); ctx.fillRect(4, 8, 6, 2);

      ctx.fillStyle = boosterGrad;
      ctx.fillRect(-14, -6, 6, 18); ctx.beginPath(); ctx.moveTo(-14, -6); ctx.lineTo(-8, -6); ctx.lineTo(-11, -16); ctx.fill(); 
      ctx.fillRect(8, -6, 6, 18); ctx.beginPath(); ctx.moveTo(8, -6); ctx.lineTo(14, -6); ctx.lineTo(11, -16); ctx.fill(); 

      ctx.fillStyle = cylGrad;
      ctx.fillRect(-5, -20, 10, 36);

      ctx.fillStyle = "#b0bec5";
      for(let i=0; i<4; i++) ctx.fillRect(-5, -5 + i*2, 10, 1);
    ...
    return () => {
      ...
      canvas.removeEventListener("touchstart", onPtr);
      canvas.removeEventListener("touchmove", onPtr);
    };
  }, [onOutcome, phase]); // Rebind if phase changes
